...
void VFS::init_vfs_interface()
{
    ...
    this->vfs_interface.open = []( const char *path, unsigned mode,
                                   unsigned hints ) -> struct retro_vfs_file_handle * {
    ...
    this->vfs_interface.seek = []( struct retro_vfs_file_handle *stream, int64_t offset,
                                   int seek_position ) -> int64_t {
        return RetroHost::get_singleton()->vfs.seek( stream, offset, seek_position );
    };
    this->vfs_interface.read = []( struct retro_vfs_file_handle *stream, void *s,
                                   uint64_t len ) -> int64_t {
        return RetroHost::get_singleton()->vfs.read( stream, s, len );
    };
    this->vfs_interface.write = []( struct retro_vfs_file_handle *stream, const void *s,
                                    uint64_t len ) -> int64_t {
    ...
    this->vfs_interface.truncate = []( struct retro_vfs_file_handle *stream,
    ...
    this->vfs_interface.opendir = []( const char *dir,
                                      bool include_hidden_files ) -> struct retro_vfs_dir_handle * {
    ...
    this->vfs_interface.dirent_get_name =
        []( struct retro_vfs_dir_handle *dir_stream ) -> const char * {
...
const char *VFS::get_path( struct retro_vfs_file_handle *stream )
{
    return NULL;
}

struct retro_vfs_file_handle *VFS::open( const char *path, unsigned mode, unsigned hints )
{
    return NULL;
}
...
struct retro_vfs_dir_handle *VFS::opendir( const char *dir, bool include_hidden_files )
{
    return NULL;
}

bool VFS::read_dir( struct retro_vfs_dir_handle *dir_stream )
{
    return false;
}

const char *VFS::dirent_get_name( struct retro_vfs_dir_handle *dir_stream )
{
    return NULL;
}

bool VFS::dirent_is_dir( struct retro_vfs_dir_handle *dir_stream )
{
    return false;
}

int VFS::closedir( struct retro_vfs_dir_handle *dir_stream )
{
    return -1;
}
